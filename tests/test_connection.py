#!/usr/bin/env python3
"""
Script de test de connexion MSP - SANS armement ni contrôle moteurs
...
def main():
    ...
    try:
        ...
        print("\nVérifiez :")
        print("  - Le câblage TX/RX/GND")
        print("  - Le port série : ls -l /dev/ttyAMA0")
        print("  - L'activation UART : sudo raspi-config")
        sys.exit(1)
    ...
    try:
        ...
        print("\nCela peut indiquer :")
        print("  - Mauvais baudrate (vérifiez dans iNAV Configurator)")
        print("  - Timeout MSP (câble trop long? interférences?)")
        print("  - Port UART non configuré pour MSP dans iNAV")
        drone.disconnect()
        sys.exit(1)
    ...
    print("   1. Vérifiez que toutes les valeurs sont cohérentes")
    print("   2. Si GPS nécessaire, attendez un bon fix (8+ sats)")
    print("   3. Configurez MSP RX dans iNAV si vous voulez contrôler via MSP")
    print("   4. Testez avec drone.enable_rc_override() si contrôle RC via MSP\n")
